...
def random_batch_kernel(
    ...
    if rate_iso == 1:
        ...
        return torch.FloatTensor(kernel), sigma.squeeze() if tensor else kernel
...
def batch_jpeg_compression(image_batch, qualities):
    ...
    for i in range(B):
        ...
        compressed_image = torchvision.io.decode_jpeg(
            torchvision.io.encode_jpeg(image, quality=quality))
        compressed_image = compressed_image / 255.0
        compressed_images.append(compressed_image)
    return torch.stack(compressed_images, dim=0)
...
class SRMDPreprocessing(object):
    def __init__(
        ...
        random_degradation=True, noise=False, cuda=False, random_disturb=False,
        sig=0, sig_min=0, sig_max=0, rate_iso=1.0, rate_cln=1, noise_high=0,
        stored_kernel=False, pre_kernel_path=None, quality_set=None,
        ...
        self.sig = sig
        self.noise_high = noise_high / 255
        self.random = random_degradation
        self.quality_set = quality_set
    ...
    def __call__(self, hr_tensor):
        ...
        # downsample hr image to yield sharp version of lr image
        if self.scale != 1:
            if self.interpolate_mode == 'nearest':
                lr_sharp_t = F.interpolate(hr_var, scale_factor=1/self.scale)
            else:
                lr_sharp_t = b_Bicubic(hr_var, self.scale)
        else:
            lr_sharp_t = hr_var 

        # blurring
        if self.random:
            b_kernels, sigma = self.kernel_gen(self.random, B, tensor=True)
        else:
            b_kernels = self.kernel_gen(self.random, B, tensor=True)
            sigma = [self.sig for _ in range(B)]
        ...
        if self.scale != 1:
            if self.interpolate_mode == 'nearest':
                lr_blured_t = F.interpolate(hr_blured_var, scale_factor=1/self.scale)
            else:
                lr_blured_t = b_Bicubic(hr_blured_var, self.scale)
        else:
            lr_blured_t = hr_blured_var

        final_lr_img = lr_blured_t

        # additive gaussian noise
        # if self.noise:
        #     if self.random:
        #         noise_level = torch.from_numpy(np.random.uniform(size=B) * self.noise_high)
        #     else:
        #         noise_level = torch.tensor([self.noise_high for _ in range(B)])
        #     lr_noised_t = b_GaussianNoising(lr_blured_t, noise_level)
        #     final_lr_img = lr_noised_t

        # jpeg compression
        # if self.random:
        #     qualities = np.random.choice(self.quality_set, B, replace=True)
        # else:
        #     qualities = [self.quality_set[0] for _ in range(B)]
        # lr_compressed = batch_jpeg_compression(lr_noised_t, qualities)

        # return tensors
        lr = final_lr_img.to(device)
        lr_sharp = lr_sharp_t.to(device)
        sigma = torch.tensor(sigma).to(device)
        noise_level = noise_level.to(device) if False else None
        qualities = torch.tensor(qualities).to(device) if False else None

        return lr, lr_sharp, sigma, noise_level, qualities